...
def calcular_ear(face, p_olho_dir, p_olho_esq):
    try:
            ...
            for face_landmarks in saida_facemesh.multi_face_landmarks:
                ...
                for id_coord, coord_xyz in enumerate(face):
                    ...
                    if id_coord in p_boca:
                        coord_cv = mp_drawing._normalized_to_pixel_coordinates(coord_xyz.x, coord_xyz.y, largura, comprimento)
                        cv2.circle(frame, coord_cv, 2, (0,255,0), -1)

                ear = calcular_ear(face, p_olho_dir, p_olho_esq)
                cv2.rectangle(frame, (0, 1), (290, 140), (58, 58, 55), -1)
                cv2.putText(frame, f"EAR: {round(ear, 2)}", (1, 24),
                                cv2.FONT_HERSHEY_DUPLEX,
                                0.9, (255, 255, 255), 2)
                
                mar = calcular_mar(face, p_boca)
                ...
                piscadas_pm = 15 if tempo_decorrido == 0 else sum(contagem_lista) / len(contagem_lista)

                cv2.putText(frame, f"Contagem de piscadas: {contagem_piscadas}", (1, 120),
                                cv2.FONT_HERSHEY_DUPLEX,
                                0.9, (109, 233, 219), 2)
